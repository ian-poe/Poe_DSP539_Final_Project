...
depth_per_year_density = function(data, year1, year2){                                      #Create Function "depth_per_year_density" to Overlay the distribution density of Depth for two chosen years 
  ...
  ggplot(df, aes(x = values, fill = group, color = group)) +                                # ggplot is used specifying variables as values (depth) on the X 
    geom_density(alpha = 0.5, linewidth = 1) +                                              # Smoothed density curves are added for calues (depth)
    ...
    labs(title = paste("Comparison of Depth Distributions:", year1, "vs.", year2),          #Title which changes with inputed years, X and Y axis lables
         x = "Depth",
         y = "Density") +
    theme_minimal()
}
...
species_of_intrest = c(species_decreased,species_increased)

station = read.csv("../Final_Project/station.csv")
catch  = read.csv("../Final_Project/catch.csv")
# ctd = read.csv("../Final_Project/ctd.csv")
# ctd_proc_cast = read.csv("../Final_Project/ctd_proc_cast.csv")

dwh_longline = left_join(catch,station,by = "STATIONKEY")

Longline_Species_of_Intrest = dwh_longline %>% filter(TAXON %in% species_of_intrest)

print(Longline_Species_of_Intrest)

species_counts <- Longline_Species_of_Intrest %>%
  count(TAXON)
...
longline_max_species_list = species_counts$TAXON

longline_top_species_seen <- dwh_longline %>%
  filter(TAXON %in% longline_max_species_list)
longline_top_species_seen$year = substring(longline_top_species_seen$START_TIME,6,7)

longline_top_species_seen <- longline_top_species_seen %>%
  mutate(
    year = ifelse(as.numeric(year) < 25, 
                            paste0("20", year), 
                            paste0("19", year))
  )

mean_longline_top_species <- longline_top_species_seen %>%
    group_by(year,TAXON) %>%
...
ggplot(mean_longline_top_species, aes(x = year, y = mean_number)) +
  geom_point() +  # Scatter plot
  geom_line() +   # Add lines for trends
  facet_wrap(~ TAXON) +  # Separate plots by species
  labs(title = "Species Observations Over Time",
       x = "Year",
       y = "Mean Number Observed") +
  theme_minimal()

mean_longline_top_species <- mean_longline_top_species %>%
  mutate(after_spill = ifelse(year >= 2010, 1, 0)) 

model = lm(mean_number ~ year + TAXON + after_spill   ,data = mean_longline_top_species)
